...
<template>
  <div class="field-wrapper" :class="{'design-time-bottom-margin': !!this.designer}">
    <el-form-item v-if="!!field.formItemFlag && (!field.options.hidden || (designState === true))"
      ...
      <!-- <br v-if="!isReadMode"/> -->
      <div v-if="field.options.descInfo && !isReadMode" :class="{'setMarginTop':field.options.descInfo}" style="color:rgb(170, 170, 170)" v-html="field.options.descInfo.replace(/<([/]?)(p)((:?s*)(:?[^>]*)(:?s*))>/g, '<$1div$3>')"></div>
      <slot></slot>
    </el-form-item>
...
<script>
  ...
  export default {
    ...
    inject: ['getFormConfig', 'getSubFormFieldFlag','getReadMode', 'getSubFormName', 'getObjectFieldFlag', 'getObjectName'],
    computed: {
      formConfig() {
      ...
      isReadMode() {
        // console.log("sss>",!!this.getReadMode() ? true : false)
        //return this.getReadMode() || this.fieldReadonlyFlag
        return !!this.getReadMode() ? true : false
      },
