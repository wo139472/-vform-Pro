  ...
  export default {
    ...
    methods: {
      ...
      chooseFile(file, fileList) {
        ...
        // let uploadFileTypes = ['jpeg','jpg','png','gif']
        // fileTypeCheckResult = uploadFileTypes.some( (ft) => {
        //   return extFileName.toLowerCase() === ft.toLowerCase()
        // })
        // if (fileTypeCheckResult && !this.field.options.setFileType) {
        //   fileList.pop()
      ...
      async uploadFile(file, fileList) {
        ...
        const res = await this.$http.post('/Upload/uploadFile',formData
        // {
        //   headers:{
        ...
        const {data} = res.data
        if(res.data.success){
          this.fileList.push({name:data.filename,url:data.filepath_url,length:data.size,uid:file.uid})
      ...
      removeUploadFile(fileName, fileUrl, fileUid) {
        ...
        this.fileList.map((fi, idx) => {
          if ((fi.name === fileName) && ((fi.url === fileUrl) || (!!fi.uid && fi.uid === fileUid))) {  /* 这个判断有问题？？ */
            foundFileIdx = idx
          }
        })
        if (foundFileIdx > -1) {
        ...
        // let old = deepClone(this.field.options.fileList)
